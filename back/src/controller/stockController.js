  ...
  try {
    if (!quantiteDisponible || !resource) {
      return res
        ...
        .json({
          message:
            "Required fields are missing. Quantite disponible and resource must be provided.",
        });
    }
    ...
    res
      .status(201)
      .json({ message: "Stock added successfully", data: savedStock });
  } catch (error) {
...
const getAllStocks = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: "Stocks retrieved successfully", data: stocks });
  } catch (error) {
