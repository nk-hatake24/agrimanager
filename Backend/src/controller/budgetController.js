...
const deleteBudget = async (req, res) => {
  ...
  const searchAllBudget = async (req, res) => {
    try {
      const budget = await Budget.find({}, { password: 0 });
      return res
        .status(200)
        .json({ message: "all the budget search", data: budget });
    } catch (error) {
...
module.exports = {
    addBudget, updateBudget, deleteBudget, searchAllBudget
}